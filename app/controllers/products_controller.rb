class ProductsController < ApplicationController
 ...
 # layout 'application_internal', :only => [:index]
  before_filter :authenticate_user!, :except => [:index, :auto_complete_for_product_name, :show, :case_studies, :report_settings_get, :produce_psa_report, :api_produce_psa_report]
  ...
  def auto_complete_for_product_name
    ...
    @items = Product.find_with_ferret("#{product_name}*", {:limit => 15}) if @items.blank?
...
#        @report = current_user.product_report
#      else
#        @report = current_user.product_reports.find(params[:product_report_id])
#      end
...
#      @product_group = current_user.product_groups.find params[:product_group_id]
      ...
      if !params[:product].blank?
        ...
        @products = Product.find_with_ferret("#{params["product"]}*", :page => params[:page].blank? ? 1 : params[:page], :per_page => 15) if @products.blank?
      else
        @products = [].paginate
      end
...
#### TRENDING REPORT #######################
  ...
  def get_one_product_group
   group = ProductGroup.find_by_name params[:id]
  # raise "group access denied" if group.user_id!=current_user.id
	# raise "not one product group" if !group.one_product_group
	 group
  end
